...
def prim(graph,start):
    MST = set()
    # MST에서 다른 정점으로 가는 비용을 저장하는 배열
    # weight = graph[start][:]
    weight = [0xffffff]*V
    # 0번에서 시작할거니까..0번까지 가는 비용은 0
    weight[start] = 0
    # MST에 모든 정점이 포함될 때까지 정점 추가 하기 반복
    while len(MST) < V:
        ...
        MST.add(min_node)
        # 새로운 노드가 추가 되었으니...
        # 그 노드로 부터 다른 정점까지 가는 비용을 다시 계산
        # 계산한 비용이 weight의 비용보다 작으면
        # 계산한 비용으로 weight의 비용을 수정
        for i in range(V):
